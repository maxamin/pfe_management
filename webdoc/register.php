...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["username"]))) {
        ...
        if (!ctype_alnum(str_replace(array("@", "-", "_"), "", $username))) {
            ...
            $dsn = 'sqlite:database.db';  # Replace 'database.db' with the actual path to your SQLite database file
            $pdo = new PDO($dsn);
    ...
    if (empty($username_err) && empty($email_err) && empty($password_err)) {
        ...
        $sql = "INSERT INTO users(username, email, password,departement) VALUES (:username, :email, :password,:departement)";

        if ($stmt = $pdo->prepare($sql)) {
            # Bind variables to the prepared statement as parameters
            $stmt->bindParam(":username", $param_username, PDO::PARAM_STR);
            $stmt->bindParam(":departement", $param_priv, PDO::PARAM_STR);
            $stmt->bindParam(":email", $param_email, PDO::PARAM_STR);
